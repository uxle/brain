...
use brain_metric::classification::auc::{roc_auc_score, pr_auc_score};
...
fn test_roc_auc_score_perfect_ranking() {
    ...
    assert!((auc - 1.0).abs() < 1e-6, "Perfect ranking must yield AUC = 1.0");
...
fn test_roc_auc_score_inverted_ranking() {
    ...
    let auc = roc_auc_score(&probs, &targets);
    assert!((auc - 0.0).abs() < 1e-6, "Inverted ranking must yield AUC = 0.0");
}
