...
pub fn build_cnn_graph(in_channels: usize, out_classes: usize) -> GraphIr {
    ...
    let w_conv = b.add_constant(
        "conv_w",
        vec![16, in_channels, 3, 3],
        vec![0.01; 16 * in_channels * 9],
    );
    ...
    let w_fc = b.add_constant(
        "fc_w",
        vec![16 * 26 * 26, out_classes],
        vec![0.01; 16 * 26 * 26 * out_classes],
    );
