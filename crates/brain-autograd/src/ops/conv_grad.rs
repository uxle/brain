...
pub fn grad_conv2d(
    ...
    let dinput = Tensor::zeros(input.shape().to_vec());
    let dweight = Tensor::zeros(weight.shape().to_vec());
    let mut sum_b = red_t::sum_along_dim(grad_output, 0, false);
    while sum_b.ndim() > 1 {
        sum_b = red_t::sum_along_dim(&sum_b, 1, false);
    }
    let dbias = Some(sum_b);
