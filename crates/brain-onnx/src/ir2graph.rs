...
pub fn lower_to_graph_ir(model: &OnnxModel) -> OnnxResult<GraphIr> {
    let name = if model.graph.name.is_empty() {
        "onnx_model"
    } else {
        &model.graph.name
    };
    ...
    for (val_name, val) in &model.graph.values {
        if val.is_initializer {
            let data = val
                ...
                .map(|t| t.to_vec())
                .unwrap_or_default();
            let shape = if val.shape.is_empty() {
                vec![1]
            } else {
                val.shape.clone()
            };
    ...
    for node in &model.graph.nodes {
        ...
        let node_inputs: Vec<_> = node
            .inputs
            .iter()
            .filter_map(|inp| value_map.get(inp).copied())
            .collect();
        let out_name = node
            .outputs
            .first()
            .cloned()
            .unwrap_or_else(|| format!("{}_out", node.name));
    ...
    builder
        .build()
        .map_err(|e| OnnxError::GraphLoweringError(e.to_string()))
}
