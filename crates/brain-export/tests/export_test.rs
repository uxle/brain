...
fn test_brain_binary_container_roundtrip() {
    ...
    assert_eq!(loaded.metadata.get("spatial_layout").unwrap(), "cubic_mesh_10x10x10");

    assert_eq!(loaded.node_coords.get("encoder.attn.q").unwrap(), &NodeCoord3D::new(0, 0, 0));
    assert_eq!(loaded.node_coords.get("encoder.attn.k").unwrap(), &NodeCoord3D::new(0, 1, 0));
    assert_eq!(loaded.node_coords.get("encoder.attn.v").unwrap(), &NodeCoord3D::new(0, 2, 0));
...
fn test_bn_crc_tamper_detection() {
    ...
    model.add_tensor("w", Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0], vec![4]), None);
    ...
    assert!(result.is_err(), "Tampered .bn file must fail CRC validation");
}
