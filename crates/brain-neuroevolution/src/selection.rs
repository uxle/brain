...
mod tests {
    ...
    #[test]
    fn test_selection_stress_001() {
        let mut rng = FastRng::seed(1 as u64);
        let mut inds = Vec::new();
        for i in 0..10 {
            let mut g = Genome::new(vec![i as f64]);
            g.fitness = Some(i as f64);
            inds.push(g);
        }

        let selected = tournament_select(&inds, 5, &mut rng);
        assert!(selected.fitness.unwrap() >= 0.0);
    }

    #[test]
    fn test_selection_stress_002() {
