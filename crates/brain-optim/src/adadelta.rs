...
mod tests {
    ...
    #[test]
    fn test_adadelta_stress_261() {
        let group = ParamGroup::new(vec![0], 1.0);
        let mut opt = Adadelta::new(vec![group], AdadeltaConfig::default());

        let mut p = vec![Tensor::from_slice(&[261 as f64 * 0.1], vec![1])];
        let g = vec![Tensor::from_slice(&[0.05], vec![1])];
    ...
    #[test]
    fn test_adadelta_stress_262() {
        let group = ParamGroup::new(vec![0], 1.0);
        let mut opt = Adadelta::new(vec![group], AdadeltaConfig::default());

        let mut p = vec![Tensor::from_slice(&[262 as f64 * 0.1], vec![1])];
        let g = vec![Tensor::from_slice(&[0.05], vec![1])];
