...
use brain_loss::regression::mse::{MSELoss, MAELoss, HuberLoss};
...
fn test_cross_entropy_loss_computation() {
    ...
    assert!(loss_val > 0.0 && loss_val < 1.0, "Loss for confident correct prediction should be small");
}
...
fn test_mse_mae_huber_losses() {
    ...
    let mse = MSELoss { reduction: Reduction::Mean };
